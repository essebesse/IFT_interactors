  ...
  for (const {bait_uniprot, prey_uniprot, validation} of YANG_2020_VALIDATIONS) {
    try {
      ...
      if (existingValidation) {
        ...
        const hasYangStudy = existingValidation.experimental_methods?.some(
          method => method.study?.includes("Yang") && method.study?.includes("2020")
        );
        ...
        console.log(`🔄 Updated: ${row.bait} ↔ ${row.prey} (added Yang 2020 to existing validation)`);
        added++;
      } else {
